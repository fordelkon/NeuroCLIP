# Train cross-subject baseline (without KG) for subjects 01-10
# Usage: .\train_cross_loop.ps1 [model]
#
# Arguments:
...
# Examples:
#   .\train_cross_loop.ps1
#   .\train_cross_loop.ps1 nice
#   .\train_cross_loop.ps1 atms
...
for ($i = 1; $i -le 10; $i++) {
    ...
    Write-Host "Training cross-subject baseline for $subject with $Model" -ForegroundColor Cyan
    Write-Host "=========================================" -ForegroundColor Cyan
    ...
    uv run --no-sync python .\src\train.py `
        ...
        logger=csv `
        hydra.run.dir=logs/train/runs/${Model}_cross/${Model}_$subject `
        extract=thingseeg2 `
        build_kg=thingseeg2
