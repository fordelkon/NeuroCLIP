...
# Usage:
#   bash scripts/linux/train_parallel.sh <subject_id> <model> <num_folds> [hydra_overrides...]
#
# Description:
...
#
# Examples:
#   bash scripts/linux/train_parallel.sh sub-10 flatnet 5
#   bash scripts/linux/train_parallel.sh sub-01 nice 10 trainer=gpu

set -euo pipefail

if [ $# -lt 3 ]; then
  echo "Error: Missing required arguments"
  echo "Usage: bash scripts/linux/train_parallel.sh <subject_id> <model> <num_folds> [hydra_overrides...]"
  exit 1
fi
...
for ((fold=0; fold<NUM_FOLDS; fold++)); do
  bash scripts/linux/train.sh "$SUBJECT_ID" "$MODEL" "$NUM_FOLDS" "$fold" "$@" &
