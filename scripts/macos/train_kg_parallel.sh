...
# Usage:
#   bash scripts/macos/train_kg_parallel.sh <subject_id> <model> <num_folds> [hydra_overrides...]
#
# Description:
...
#
# Examples:
#   bash scripts/macos/train_kg_parallel.sh sub-10 flatnet 5
#   bash scripts/macos/train_kg_parallel.sh sub-01 nice 10 model.kgnet.alpha=0.5

set -euo pipefail

if [ $# -lt 3 ]; then
  echo "Error: Missing required arguments"
  echo "Usage: bash scripts/macos/train_kg_parallel.sh <subject_id> <model> <num_folds> [hydra_overrides...]"
  exit 1
fi
...
for ((fold=0; fold<NUM_FOLDS; fold++)); do
  bash scripts/macos/train_kg.sh "$SUBJECT_ID" "$MODEL" "$NUM_FOLDS" "$fold" "$@" &
