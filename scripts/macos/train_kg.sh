...
# Usage:
#   bash scripts/macos/train_kg.sh <subject_id> <model> <k_fold> <fold_idx> [hydra_overrides...]
#
# Description:
...
#
# Examples:
#   bash scripts/macos/train_kg.sh sub-01 flatnet 5 0
#   bash scripts/macos/train_kg.sh sub-02 nice 10 1
#   bash scripts/macos/train_kg.sh sub-03 atms 5 0 model.kgnet.alpha=0.5

set -euo pipefail

if [ $# -lt 4 ]; then
  echo "Error: Missing required arguments"
  echo "Usage: bash scripts/macos/train_kg.sh <subject_id> <model> <k_fold> <fold_idx> [hydra_overrides...]"
  exit 1
fi
